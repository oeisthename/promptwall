...
def main(
    ...
    if remove_limit:
        ...
        if "daily_budget" in config:
            ...
            save_config(config)
        console.print(
            "[green]✔[/green] Daily budget limit removed (now [bold cyan]Unlimited[/bold cyan])"
        )
    ...
    table.add_row(
        "[dim]Daily Limit:[/dim]",
        f"[bold cyan]${limit:0.2f}[/bold cyan]" if limit else "[dim]Unlimited[/dim]",
    )
    table.add_row(
        "[dim]Today's Spend:[/dim]",
        f"[bold {'red' if limit and today_cost >= limit else 'green'}]${today_cost:0.4f}[/]",
    )
