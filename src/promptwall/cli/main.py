...
def about() -> None:
    ...
    info_text = """
[bold cyan]PromptWall[/bold cyan] is an advanced, enterprise-grade runtime security middleware designed specifically to shield AI agents and LLMs from sophisticated prompt injections, adversarial jailbreaks, and unauthorized data exfiltration. Deployed at the edge, it provides low-latency enforcement of active security policies.
...
1. [cyan]promptwall init[/cyan]   - Connect your edge node to the Dashboard by providing your API Key.
2. [cyan]promptwall status[/cyan] - View the currently active security policies enforced in your environment.
3. [cyan]promptwall test[/cyan]   - Launch an interactive playground to test prompts against your live policies.

4. [cyan]promptwall serve[/cyan]  - Start the secure reverse proxy on localhost to protect your AI apps.
5. [cyan]promptwall help[/cyan]   - Show the full help menu and available options.

[dim]For comprehensive documentation and integration guides, visit the GitHub repository.[/dim]
"""
...
def init() -> None:
    ...
    console.print(PROMPTWALL_ART, overflow="crop", no_wrap=True)
    console.print(Panel.fit("[bold white]Runtime Security Middleware for AI Agents[/bold white]\n[dim]Let's connect your edge node to the Dashboard.[/dim]\n\n[bold magenta]Developed by OTHMANE EL MQIDDEM[/bold magenta]", border_style="cyan"))
...
@app.command()
def status(environment: str = typer.Option("production", "--environment", "-e", help="Environment to check")) -> None:
    """Check connection and view active policies."""
    api_key = get_api_key()
    base_url = get_base_url()
    
    if not api_key or not base_url:
        console.print("[bold red]Not configured![/bold red] Run [cyan]promptwall init[/cyan] first.")
        raise typer.Exit(1)
    ...
    for rule in rules:
        action_style = "red" if rule.get("action") == "block" else "yellow" if rule.get("action") in ("redact", "sanitize") else "green"
        table.add_row(
...
@app.command()
def test(environment: str = typer.Option("production", "--environment", "-e", help="Environment to test against")) -> None:
    ...
    while True:
        try:
            ...
            if decision.action == "allow":
                console.print(Panel(f"[bold green]ALLOWED[/bold green]\n{prompt_text}", border_style="green", title="Result"))
            elif decision.action in ("block", "require_approval"):
                console.print(Panel(f"[bold red]{decision.action.upper()}[/bold red]\nReason: {decision.reason}", border_style="red", title="Result"))
            else:
                # sanitize or redact
                console.print(Panel(f"[bold yellow]{decision.action.upper()}[/bold yellow]\nReason: {decision.reason}\n\n[bold]Sanitized Content:[/bold]\n{decision.sanitized_content}", border_style="yellow", title="Result"))
...
def serve(
    ...
    target: str = typer.Option(None, help="Default dynamic upstream provider URL (overrides config)")
) -> None:
